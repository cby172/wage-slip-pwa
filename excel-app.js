...
const REQUIRED_SHEETS = {
  工人资料: ["姓名", "入职日期", "起始底薪", "手动底薪", "加薪策略", "是否仓管", "是否在职", "工资条备注", "备注"],
  中断记录: ["姓名", "中断开始", "中断结束", "备注"],
  规则参数: ["参数", "值", "说明"]
};
const RAISE_STRATEGIES = new Set(["anniversary", "spring_holiday"]);
...
function initExcelTabs() {
  document.querySelectorAll(".excel-tabs .tab").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      document.querySelectorAll("#excel-panel-payroll, #excel-panel-data, #excel-panel-worker-stats").forEach((item) => item.classList.remove("active"));
...
function notifyParentHeight() {
  ...
  parent.postMessage({ type: "excel-height", height }, location.origin);
}
...
function parseWorkbook(workbook, fileName) {
  ...
  assertHeaders(workerSheet, "工人资料");
  if (interruptionSheet) assertHeaders(interruptionSheet, "中断记录");
  if (rulesSheet) assertHeaders(rulesSheet, "规则参数");
...
function assertHeaders(sheet, sheetName) {
  const rows = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: "" });
  const headers = (rows[0] || []).map((value) => text(value));
  const missing = REQUIRED_SHEETS[sheetName].filter((field) => !headers.includes(field));
  if (missing.length) throw new Error(`${sheetName} 表缺少字段：${missing.join("、")}`);
}
...
function parseWorkerRow(row, rowNumber, interruptionsByName, parsedRules) {
  ...
  if (!RAISE_STRATEGIES.has(raiseStrategy)) throw new Error(`工人资料第 ${rowNumber} 行：加薪策略只能填“入职日满年加薪”或“过年放假日加薪”`);
  const startSalary = parseRequiredNumber(row["起始底薪"], parsedRules.defaultBaseSalary, `工人资料第 ${rowNumber} 行：起始底薪必须是数字`);
  const overrideSalary = parseOptionalStrictNumber(row["手动底薪"], `工人资料第 ${rowNumber} 行：手动底薪必须是数字，空着表示自动计算`);
  return {
    id: `excel-${name}`,
    owner_id: "excel-local",
    name,
    hire_date: hireDate,
    base_start_salary: startSalary,
    base_salary_override: overrideSalary,
...
function parseInterruptionsSheet(rows) {
  ...
  rows.forEach((row, index) => {
    ...
    if (parseDate(end) < parseDate(start)) throw new Error(`中断记录第 ${index + 2} 行：中断结束不能早于中断开始`);
...
function parseRules(rows) {
  ...
  rows.forEach((row) => {
    ...
    if (key === "春节放假日期覆盖") {
      ...
      next[key] = parseRequiredNumber(value, next[key], `规则参数 ${key} 的值必须是数字`);
    }
  });
  return next;
}
...
function renderWorkers() {
  $("workerList").innerHTML = workers.length
    ? workers.map((worker) => {
      ...
      const manualSalary = worker.base_salary_override ? `手动底薪 ${money(worker.base_salary_override)}` : "自动计算底薪";
      const interruptionCount = (worker.interruptions || []).length;
      return `<article class="worker-card">
        <div class="worker-head">
          <div>
            ...
            <p class="meta">入职 ${escapeHtml(worker.hire_date || "未填")} · 起始底薪 ${money(worker.base_start_salary || rules.defaultBaseSalary)} · ${manualSalary} · ${worker.is_warehouse_manager ? "仓管" : "非仓管"} · 中断 ${interruptionCount} 条</p>
          </div>
        </div>
...
function renderDataHealth() {
  const target = $("dataHealth");
  if (!target) return;
  const health = validateCurrentData();
  const issueClass = health.errors.length ? "warn" : "ok";
  const warnings = health.warnings.length ? health.warnings : ["暂无明显提醒"];
  const errors = health.errors.length ? health.errors : ["字段完整，暂无明显错误"];
  target.innerHTML = `<section class="rule-note data-health-card ${issueClass}">
    <h3>Excel 数据检查</h3>
    <div class="summary-grid">
      <div class="summary-item">工人总数<b>${health.summary.total}</b></div>
      <div class="summary-item">在职工人<b>${health.summary.active}</b></div>
      <div class="summary-item">中断记录<b>${health.summary.interruptions}</b></div>
    </div>
    <h4>必须处理</h4>
...
function validateCurrentData() {
  ...
  workers.forEach((worker, index) => {
    const label = worker.name || `第 ${index + 1} 个工人`;
    const name = text(worker.name);
    if (!name) errors.push(`工人资料第 ${index + 1} 条缺少姓名`);
    if (name) names.set(name, (names.get(name) || 0) + 1);
    if (!parseDate(worker.hire_date)) errors.push(`${label} 的入职日期为空或格式错误`);
    if (!Number.isFinite(Number(worker.base_start_salary))) errors.push(`${label} 的起始底薪不是数字`);
    if (worker.base_salary_override !== null && worker.base_salary_override !== undefined && worker.base_salary_override !== "" && !Number.isFinite(Number(worker.base_salary_override))) {
      errors.push(`${label} 的手动底薪不是数字`);
    }
    if (!RAISE_STRATEGIES.has(worker.raise_strategy)) errors.push(`${label} 的加薪策略不正确`);
    if (!worker.slip_note) warnings.push(`${label} 没有工资条备注，生成时会用默认备注`);
    if (Number(worker.base_salary_override || 0) > Number(rules.salaryCap || defaultRules.salaryCap)) {
      warnings.push(`${label} 的手动底薪超过底薪封顶，计算时会按封顶处理；职位津贴另算`);
    }
    (worker.interruptions || []).forEach((item, itemIndex) => {
      interruptionCount += 1;
      const start = parseDate(item.start);
      const end = parseDate(item.end);
      if (!start || !end) errors.push(`${label} 第 ${itemIndex + 1} 条中断记录日期格式错误`);
      else if (end < start) errors.push(`${label} 第 ${itemIndex + 1} 条中断记录结束早于开始`);
    });
  });

  [...names.entries()].filter(([, count]) => count > 1).forEach(([name]) => errors.push(`工人姓名重复：${name}`));
  if (!activeWorkers.length) errors.push("没有在职工人，无法生成正常工资条");

  const warehouseCount = activeWorkers.filter((worker) => worker.is_warehouse_manager).length;
  if (warehouseCount === 0) warnings.push("当前没有在职仓库管理员，职位津贴不会发给任何人");
  if (warehouseCount > 1) warnings.push(`当前有 ${warehouseCount} 个在职仓库管理员，都会计算职位津贴`);
...
function parseRequiredNumber(value, fallback, message) {
...
function parseOptionalStrictNumber(value, message) {
  const raw = text(value);
  if (!raw) return null;
  const number = Number(raw);
  if (!Number.isFinite(number)) throw new Error(message);
...
function parseRaiseStrategy(value) {
  const raw = text(value);
  if (!raw || raw === "入职日满年加薪" || raw === "anniversary") return "anniversary";
