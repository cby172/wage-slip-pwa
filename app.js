...
function buildPayroll(year, month, rawInput) {
  ...
  const allText = [header, "", ...rows.flatMap((row) => [row.slipText, ""]), `应发合计：${money(totalGross)}元。`, `扣预支后实发合计：${money(totalNet)}元。`].join("\n");
  return {
    year,
    month,
    monthDays,
...
function buildWorkerPayroll(worker, record, year, month, monthDays, segments) {
  ...
  const grossBeforeLeave = roundYuan((monthlySalary / monthDays) * segmentDays);
  const leaveStats = calculateLeaveDeduction(record, year, month, monthlySalary, monthDays, segments);
  const gross = Math.max(0, roundYuan(grossBeforeLeave - leaveStats.totalDeduction));
  const net = Math.max(0, roundYuan(gross - record.advance));
  const lines = [];
  const note = worker.raise_strategy === "spring_holiday" ? "过年放假日加薪" : "入职日满年加薪";
  lines.push(`@${worker.name}${money(monthlySalary)}(${note})`);
  lines.push(`${month}月份工资`);
  if (salary.allowance > 0) {
    lines.push(`(基本工资${money(salary.baseSalary)}+仓管津贴${money(salary.allowance)}=${money(monthlySalary)})`);
  }
  ...
  if (segmentDays !== monthDays || leaveStats.totalDeduction > 0) {
    lines.push(`${money(monthlySalary)}/${monthDays}×${formatDays(segmentDays)}天=${money(grossBeforeLeave)}元`);
  }
  if (leaveStats.totalDeduction > 0) {
    lines.push(`请假扣款：${money(leaveStats.totalDeduction)}元`);
  }
  if (record.advance > 0) {
    lines.push(`${money(gross)}-预支${money(record.advance)}=${money(net)}元`);
  }
  lines.push(`实发：${money(net)}元。`);
  return { worker, record, salary, monthlySalary, gross, net, leaveStats, segments, slipText: lines.join("\n") };
}
...
function calculateLeaveDeduction(record, year, month, monthlySalary, monthDays, segments) {
  ...
  const regularDeductDays = regularLeave <= rules.paidLeaveLimit
    ? Math.max(regularLeave - rules.paidLeaveDays, 0)
    : regularLeave;
...
function formatLeaveLine(record, stats) {
  ...
  const morning = record.items.filter((item) => item.type === "morning").map((item) => item.day);
  const afternoon = record.items.filter((item) => item.type === "afternoon").map((item) => item.day);
  const parts = [];
  if (full.length) parts.push(`${full.join("/")}号${full.length > 1 ? "各" : ""}一天`);
  if (morning.length) parts.push(`${morning.join("/")}号上午${morning.length > 1 ? "各" : ""}半天`);
  if (afternoon.length) parts.push(`${afternoon.join("/")}号下午${afternoon.length > 1 ? "各" : ""}半天`);
  const suffix = stats.doubleLeave > 0
    ? `(普通请假${formatDays(stats.regularLeave)}天，双薪请假${formatDays(stats.doubleLeave)}天，扣${formatDays(stats.regularDeductDays)}天+双薪${formatDays(stats.doubleLeave)}天)`
    : stats.regularLeave <= rules.paidLeaveLimit
      ? `(有薪${formatDays(Math.min(stats.regularLeave, rules.paidLeaveDays))}请${formatDays(stats.regularDeductDays)})`
      : `(请${formatDays(stats.regularDeductDays)})`;
  return `请假：${parts.join("，")}${suffix}`;
}
